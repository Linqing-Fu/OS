...
int main(int argc, char *argv[]){
	...
	Elf32_Phdr* proc_phdr[3];
	Elf32_Ehdr *kernel_ehdr, *boot_ehdr;
	Elf32_Ehdr* proc_ehdr[3];
	...
	write_file(&image_file, kernel_file, kernel_ehdr, kernel_phdr, 127);
	...
	for(i = 0; i < argc -4; i++){
		...
		total_sec += 128;
		write_file(&image_file, proc_file[i], proc_ehdr[i], proc_phdr[i], 128);
	}
	//printf("total sector: %d\n", total_sec);
	record_kernel_sectors(&image_file, kernel_ehdr, kernel_phdr, 512);

	/*print infomation if have '--extended' */
	if (!strcmp(argv[1],"--extended")){
		extended_opt(boot_file, kernel_file, boot_phdr, kernel_ehdr->e_phnum, kernel_phdr, 512);
	}
