...
void unblock_all(node_t * wait_queue);
void mail_unblock(pcb_t *pcb);
void release_all_lock(bool_t *lock);
...
static void init_syscalls()
{
	...
	syscall[SYSCALL_LOCK_INIT] =  &my_lock_init;
	syscall[SYSCALL_LOCK_ACQUIRE] =  &my_lock_acquire;
	syscall[SYSCALL_LOCK_RELEASE] =  &my_lock_release;
...
static int do_spawn(const char *filename)
{
  ...
  /*initialize pcb, put into ready_queue*/
  enter_critical();
  pid = next_pid++;
  leave_critical();
  p = &pcb[num];
...
static int do_kill(pid_t pid)
{
  ...
  if (pid == current_running->pid){
  	release_all_lock(pcb[num].lock);
  	enter_critical();
  	current_running->status = EXITED;
  	unblock_all(&(pcb[num].wait_queue));
  	mail_unblock(&pcb[num]);
  	scheduler_entry();
  	//leave_critical();
  } else {
  	release_all_lock(pcb[num].lock);
...
void mail_unblock(pcb_t *pcb){
	...
	bool_t temp;
	for(i = 0; i < MAX_MBOXEN; i++){
		if(pcb->mailbox[i] == TRUE){
			temp = current_running->mailbox[i];
			do_mbox_close(i);
			current_running->mailbox[i] = temp;
		}
	}
}
...
/*put current in other's pcb.wait_queue*/
  enter_critical();
  current_running->status = BLOCKED;
  enqueue(&pcb[i].wait_queue, (node_t*)current_running);
  scheduler_entry();
  //leave_critical();
  return 0;
}
