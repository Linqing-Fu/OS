...
int my_lock_init(){
  int i;
  for (i = 0; i < 32; i ++){
    lock_init(&kernel_lock[i]);
  }
  return 0;
}

int my_lock_acquire(int i){
  if (i>=0 && i<= 32){
    lock_acquire(&kernel_lock[i]);
    current_running->lock[i] = TRUE;
  }
  return 0;
}

int my_lock_release(int i){
  if (i >= 0 && i<= 32){
    lock_release(&kernel_lock[i]);
    current_running->lock[i] = FALSE;
  }
  return 0;
}
...
void condition_wait(lock_t * m, condition_t * c){
  ...
  /*block current task*/
  block(&c->wait_queue);
  lock_acquire_helper(m);/* when put into ready_queue again and be schedulered*/
...
void condition_signal(condition_t * c){
  ...
  /*
  p = (pcb_t *)dequeue(&c->wait_queue);
  if (p == NULL){
    ;
  } else {
    unblock(p);
    //(current_running->primitive).c = NULL;
  }
  */
  leave_critical();
...
void semaphore_up(semaphore_t * s){
  ...
  if (s->value <= 0 && !is_empty(&s->wait_queue)){
    ...
    s->value ++;
    //current_running->primitive.s = NULL;
  }
...
void barrier_wait(barrier_t * b){
  ...
  if(b->size +1 >= b->num_barrier){
    b->size = 0;
    unblock_all(&b->wait_queue);
  } else {
    b->size ++;
    //current_running->primitive.b = b;
    block(&b->wait_queue);
  }
