...
{
	int *buf = (int *)malloc(512*sizeof(int));
	printf("buf = %d", (uint32_t)buf);
...
int main(int argc, char *argv[]){
	...
	//printf("image = %d", (uint32_t)image_file);
	...
	write_file(&image_file, kernel_file, kernel_ehdr, kernel_phdr, 50);

	for(i = 0; i < argc -4; i++){
		proc_phdr[i]  = malloc(sizeof(Elf32_Phdr));

		//printf("proc_phdr[%d] = %d", i, (uint32_t)proc_phdr);

		proc_phdr[i] = read_exec_file(&proc_file[i], argv[i+4], &proc_ehdr[i]);
		//proc_num_sec[i]=count_kernel_sectors(proc_ehdr[i], proc_phdr[i]);
		total_sec += 4;
		write_file(&image_file, proc_file[i], proc_ehdr[i], proc_phdr[i], 7);
	}
	//printf("total sector: %d\n", total_sec);
	record_kernel_sectors(&image_file, kernel_ehdr, kernel_phdr, 64);
